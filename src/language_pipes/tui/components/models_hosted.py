...
class ModelsHosted:
    ...
    def add_model(self):
        valid_device_name = self.loader.call_provider(ProviderCall.validate_device_name, self.edit_device_name)
        if not self.validate_memory() or not valid_device_name or self.edit_model_id is None:
            return
        model = ModelToLoad(
