...
class ModelsLayerModels:
    ...
    def _on_next(self):
        if self.state == LayerModelsState.Edit:
            self.edit_idx = (self.edit_idx + 1) % (self._max_edit_idx() + 1)
    ...
    def get_view(self) -> List[str]:
        if self.state == LayerModelsState.ChooseModel:
            return self._get_choosing_model_view()
        elif self.state == LayerModelsState.ChooseDevice:
            return self._get_choosing_device_view()
        elif self.state == LayerModelsState.Edit:
            return self._get_editor_view()
        elif self.state == LayerModelsState.Options:
            return self._get_options_view()
        else:
            return self._get_list_view()
        
    def _get_options_view(self) -> List[str]:
        model = self.get_editing_model()
        if model is None:
            return ["ERROR"]
    ...
    def _get_choosing_device_view(self) -> List[str]:
        lines = ["Choose Device", ""]
    ...
    def _get_choosing_model_view(self) -> List[str]:
    ...
    def _get_editor_view(self) -> List[str]:
    ...
    def _get_list_view(self) -> List[str]:
    ...
    def _network_not_started_warning(self) -> List[str]:
    ...
    def _start_edit(self) -> None:
        ...
        if model is not None and self.model_idx < len(self.layer_models):
    ...
    def _add_model(self) -> None:
        ...
        if self.editing_config_idx is None:
            ...
            self.layer_models[self.editing_config_idx] = model
    ...
    def _open_choose_device(self) -> None:
    ...
    def is_adding_model(self) -> bool:
        return self.model_idx == len(self.layer_models)
