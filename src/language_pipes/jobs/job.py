...
from language_pipes.jobs.pass_sequence import PassSequence, SavedPass
...
class Job:
    ...
    def replay(self, saved: SavedPass):
        """Take back the state this node was in when it sent that pass.

        The cache is not touched: the keys and values for the pass are in it
        already, and computing again would append them a second time.
        """
    ...
    def receive_network_job(self, network_job: NetworkJob, node_id: str) -> bool:
        ...
        # packet is the one packet that can only be a node asking the origin to
        # send the pass again.
        if network_job.data is None and network_job.compute_step == ComputeStep.EMBED:
            saved = self.passes.restart(network_job.pass_idx)
            if saved is None:
                return False
            self.replay(saved)
            return True

        saved = self.passes.accept(
            network_job.pass_idx,
            network_job.compute_step,
            network_job.current_layer
        )
        if self.passes.error is not None:
            return False

        # The origin owns the numbering; it never takes one from the wire.
        if node_id != self.origin_node_id:
            self.passes.adopt(network_job.pass_idx)
    ...
    def get_job_ram(self) -> float:
        """KV cache held for this job, in GB. The payload kept for a restart
        (`passes.outputs`) is one pass of hidden state and is not counted."""
        total_bytes = 0
        tensors = []
