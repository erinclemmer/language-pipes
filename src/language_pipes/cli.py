...
def build_parser():
    ...
    run_parser.add_argument("--config-file", help="Path to TOML config file")
...
def apply_overrides(config, args):
    ...
    def precedence(key, arg, d):
        return arg or env_map[key] or getattr(config, key, d)

    config.logging_level = precedence("logging_level", args.logging_level, "INFO")
    config.oai_port = precedence("oai_port", args.oai_port, None)
    config.node_id = precedence("node_id", args.node_id, None)
    config.peer_port = int(precedence("peer_port", args.peer_port, 5000))
    config.bootstrap_address = precedence("bootstrap_address", args.bootstrap_address, None)
    config.bootstrap_port = precedence("bootstrap_port", args.bootstrap_port, None)
    config.network_key = precedence("network_key", args.network_key, "network.key")
    config.https = precedence("https", args.https, False)
    config.job_port = int(precedence("job_port", args.job_port, 5050))
    config.hosted_models = precedence("hosted_models", args.hosted_models, None)

    if config.hosted_models is None:
        raise ValueError("hosted_models param must be supplied")

    if config.node_id is None:
        raise ValueError("node_id param is not supplied")
...
def main():
    ...
    if args.command == "create_key":
        ...
        print(data)
        # config = LMNetConfig.from_dict(data)
