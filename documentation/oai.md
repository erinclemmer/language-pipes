...
- **Reasoning models.** A leading `<think>…</think>` block is split off and returned as a separate `reasoning` output item (its text under `summary[0].text`); `output_text` and the `message` item contain only the answer. The tool-call parser is likewise tolerant of a reasoning block and of prose surrounding the JSON. When streaming, reasoning is **blocked** (held back) while it generates and emitted as a single reasoning item once `</think>` closes, after which the answer streams token-by-token — see the streaming note below.
...
- **Reasoning streaming.** With no tools and `stream=true`, a leading `<think>…</think>` block is buffered while generating (no deltas leak), then emitted at the boundary as `response.output_item.added` (a `reasoning` item) → `response.reasoning_summary_text.delta` (full reasoning) → `response.reasoning_summary_text.done` → `response.output_item.done`. The answer that follows then streams as normal `response.output_text.delta` events on a `message` item at the next `output_index`.
