...
- **Reasoning models.** Output is tolerant of a leading `<think>…</think>` reasoning block and of prose surrounding the JSON — the parser strips reasoning and recovers the first balanced JSON object. Note that for *non*-tool (plain text) responses the `<think>` block is **not** stripped from `output_text`.
