...
def check_stack(tst: unittest.TestCase, model_dir: str, cache_file: str):
    ...
    input_ids = tokenizer.apply_chat_template(chat, tokenize=True, add_generation_prompt=True, return_tensors='pt')['input_ids']
    ...
    while current_token < num_tokens:
        ...
        topk = 1
        result = StaticAutoModel.compute_head(head, input_ids, 'cpu', norm(state.state), topk)
...
def check_chunked_prefill(tst: unittest.TestCase, model_dir: str, cache_file: str, chunk_size: int = 8):
    ...
    all_input_ids = tokenizer.apply_chat_template(chat, tokenize=True, add_generation_prompt=True, return_tensors='pt')['input_ids']
    ...
    for i in range(num_decode_tokens):
        state = StaticAutoModel.compute_embedding(input_embed, current_ids, collector.config, cache)
