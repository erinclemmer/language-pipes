...
class TestContentLoader(unittest.TestCase):
    ...
    def test_load_without_provider_returns_placeholder(self):
        provider = ContentLoader(providers=None)
        result = provider.load("Network", "Status", update_status=False, force=False)
    ...
    def test_load_caches_result(self):
        provider = ContentLoader(providers=None)
        r1 = provider.load("Network", "Status", update_status=False, force=False)
        r2 = provider.load("Network", "Status", update_status=False, force=False)
        self.assertIs(r1, r2)

    def test_load_force_bypasses_cache(self):
        provider = ContentLoader(providers=None)
        r1 = provider.load("Network", "Status", update_status=False, force=False)
        r2 = provider.load("Network", "Status", update_status=False, force=True)
        # Both are placeholders but force=True creates a new dict
        self.assertIsNot(r1, r2)

    def test_invalidate_clears_specific_key(self):
        provider = ContentLoader(providers=None)
        provider.load("Network", "Status", update_status=False, force=False)
        self.assertIn(("Network", "Status"), provider._cache)
        provider.invalidate("Network", "Status")
        self.assertNotIn(("Network", "Status"), provider._cache)

    def test_invalidate_all_clears_cache(self):
        provider = ContentLoader(providers=None)
        provider.load("Network", "Status", update_status=False, force=False)
        provider.load("Network", "Peers", update_status=False, force=False)
    ...
    def test_provider_available_false_when_none(self):
        ...
        provider = ContentLoader(providers=None)
        self.assertFalse(provider.provider_available(ProviderCall.get_network_config))

    def test_provider_available_true_with_dict(self):
        from language_pipes.content_provider.provider_calls import ProviderCall
        providers = {ProviderCall.get_network_config: lambda: None}
        provider = ContentLoader(providers=providers)
        self.assertTrue(provider.provider_available(ProviderCall.get_network_config))
    ...
    def test_load_with_provider_calls_formatter(self):
        ...
        provider = ContentLoader(providers=providers)
        result = provider.load("Network", "Configure", update_status=True, force=True)
