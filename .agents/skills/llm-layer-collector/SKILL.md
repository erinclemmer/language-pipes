...
### 9) Gemma4Unified gotchas (learned)

- **Gemma4Unified is a separate architecture from Gemma4, not a variant.** `google/gemma-4-12B-it` and up ship `model_type: "gemma4_unified"` (text: `"gemma4_unified_text"`) in module `transformers.models.gemma4_unified.modeling_gemma4_unified`, with a full parallel class set (`Gemma4UnifiedTextDecoderLayer`, `Gemma4UnifiedRMSNorm`, `Gemma4UnifiedTextRotaryEmbedding`, `Gemma4UnifiedTextScaledWordEmbedding`). The E2B/E4B models remain plain `gemma4`. Registering a 12B-class checkpoint under `gemma4_text` is the natural mistake and it fails immediately.
- **The first symptom is a config error, not a modeling error.** `helpers.get_config` unwraps `text_config` by an explicit `model_type` allowlist; an unlisted wrapper config falls through and the raw multimodal config reaches `torch.nn.Embedding(config.vocab_size, ...)` → `'Gemma4UnifiedConfig' object has no attribute 'vocab_size'`. Any new multimodal wrapper needs adding to that tuple *first* — every other symptom is downstream of it.
- **No Per-Layer Embeddings.** `hidden_size_per_layer_input` is `0` and `Gemma4UnifiedTextDecoderLayer.forward` has **no** `per_layer_input` parameter. It does take `**kwargs`, so passing `per_layer_input=None` is not a clean `TypeError` — it gets forwarded into the attention call. `Gemma4Model.compute_layer` therefore adds that kwarg only when `model_type == "gemma4_text"`. Everything else (per-type masks/RoPE keyed by `layer_types`, `shared_kv_states` as a dict) is shared with Gemma4, so both types dispatch to `Gemma4Model`.
- **`attention_k_eq_v` changes head geometry on `full_attention` layers only.** When true (the real 12B setting), those layers switch to `config.global_head_dim` and `config.num_global_key_value_heads` while sliding layers keep `head_dim`/`num_key_value_heads`. A tiny spec left at the config defaults (`attention_k_eq_v=False`, `num_global_key_value_heads=None`) gives every layer uniform shapes and never exercises this path — set all three in `config_kwargs` to mirror the real checkpoint.
- **`layer_scalar`** is a real per-layer parameter here (absent in Gemma4). It loads cleanly, but since `load_layers` uses `strict=False`, confirm with the §3 state-dict check rather than assuming — `missing_keys` and `unexpected_keys` should both be empty.
- **Tied embeddings**: the checkpoint has `tie_word_embeddings: true` and ships no `lm_head.weight`, so `load_head`'s fallback to the embedding weights is the *correct* path here (unlike Ministral3, where it was a bug). `Gemma4UnifiedTextScaledWordEmbedding.weight` is unscaled — the `sqrt(hidden_size)` factor is applied in `forward` — so the tied head gets the right tensor.
- Weight keys use the same `model.language_model.*` nesting as Gemma4 (`KEY_STYLE_GEMMA4_MM`), and the 12B ships as a single `model.safetensors` rather than numbered shards.
- **Don't copy a sibling model's `expected_next`, and don't guess it.** Tier 2 now prompts through each model's own chat template (`_tokenize` in `test_real_models.py`), falling back to the raw `spec.prompt` only for base models with no template (Llama-3.2-1B). This exists because raw completion prompts make the first token an artifact of the checkpoint rather than a correctness signal: on "The capital of France is", E2B continued `' France'` but the 12B degenerated to `'1'` — both genuine behavior, neither meaningful. Copying E2B's `" France"` onto the 12B produced a *false* Tier 2 failure that looked exactly like a broken forward path.
  - **Decode several tokens, not one** (`spec.coherence_tokens`, default 8, via `greedy_decode`). A single token asserts only `"The"` — every instruct model opens its answer that way, so it tests fluency, not correctness. Eight tokens reach `'The capital of France is **Paris**.'`, which actually verifies the pipeline preserves the model's knowledge. The cost is linear: decoding is autoregressive, so each token is a full windowed reload of the decoder stack (full suite went ~350s → ~945s). Raise it only as far as the answer needs.
  - `gemma-4-12B`'s template *always* opens a `<|channel>thought` block — `thinking=False` / `enable_thinking=False` are referenced by the template but do not suppress it. It still answers within 8 tokens (its thought for a trivial factual question is essentially empty), so it *can* assert `"Paris"`. Don't assume a forced thought channel makes a model unassertable — measure it. Qwen3 does honor `enable_thinking=False` (pass it via `spec.template_kwargs`).
  - `phi3` (Phi-4-mini-reasoning) genuinely does bury the answer: 8 tokens yield `'<think>\nOkay, so the user'`. It keeps `expected_next=None`.
  - Always set `expected_next` from *observed* output, then sanity-check anything degenerate against `AutoModelForCausalLM` on the same prompt and tokenization. **If HF returns the same token you produce, the dispatch is right and the expectation is wrong.**

**A general lesson from that episode:** Tier 1 tiny parity passing while Tier 2 fails does *not* imply a real-checkpoint-only bug. Run the nearest untouched architecture as a control first — if the control passes and your new one fails, the difference is real; but the assertion itself is still a suspect until checked against HF. Ranking the cheap discriminators (control run, then HF ground truth) ahead of deep debugging saves hours of chasing a correct implementation.
